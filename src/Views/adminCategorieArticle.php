...
if(isset($categorie)) {
    $id_categorie_article = intval($categorie['id_categorie_article']);
...
<div class="form_bg">
    <div class="form_post_texte">
        ...
        <?php } ?>
        <form class="connexion_form" action="<?=HOME_URL?>admin/categorieArticle/<?= $categorie ? 'update' : 'create' ?>" method="POST">
            <?php
            if($categorie) { ?>
                <input type="hidden" name="id_categorie_article" value="<?= $id_categorie_article ?>">
            <?php } ?>
